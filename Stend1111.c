...
const char cca_std[]            = "CїeЅг ѕpoіepєё "; //Стенд прове рки
...
const char cca_tip32[]           = "PyАЅo№   ";           // Ручной ввод
  ...
  char txt2[2];
  char txt1[1];
  unsigned long  imp=0, frt=1, ch=0, cl=0, pl=0, zd=0, zb=0, Typ_izdelia=0;  //zb-число зубьеы. Остальные переменные для циклов
  int  i;      //Разряды чисел
  double float ob=0, dl=0, zh=0, nl=0.1E+0, ga=0;  //оборот колеса. длинна колеи. ширина колеи. гектары
  char tip;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 CCP1CON=0b00000101;
 CCP2CON=0b00000101;
 T1CON=0b00110001;                  //Инициализация таймера
...
strcp_c(txt_msg,"Bіeгёїe ёјѕy»ДcГ:" ); LCD_Custom_Out(1,1,txt_msg);
 imp=1;
...
break;   
        ...
        if (kp==6){break;}
        ch=ch++;                        
        PORTF.F0=0; //Отправка импульсов
        Delay_ms(9);
        PORTF.F0=1;
        Delay_ms(1);
        ga=((ch/zb)*dl*zh)/10000;//Формулы вычисления количества гектаров.
